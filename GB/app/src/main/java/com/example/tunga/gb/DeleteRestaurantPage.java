...
import retrofit2.Response;
    ...
    public class DeleteRestaurantPage extends AppCompatActivity {
        ...
        protected void onCreate(Bundle savedInstanceState) {
            ...
            setContentView(R.layout.activity_delete_restaurant_page);
            Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar);
            getSupportActionBar().setTitle("Delete Restaurant Page");

            final Intent intent = getIntent();
            int RestaurantId = intent.getIntExtra("RestaurantId", 0);


            editRestaurant = (Button) findViewById(R.id.delete_restaurant_page_edit_profile_button);
            deleteRestaurant = (Button) findViewById(R.id.delete_restaurant_page_delete_restaurant_button);
            textView = (TextView) findViewById(R.id.delete_restaurant_page_rest_name_tw);
            textView1 = (TextView) findViewById(R.id.delete_restaurant_page_rest_address_tw);
            textView2 = (TextView) findViewById(R.id.delete_restaurant_page_rest_phone_tw);
            textView3 = (TextView) findViewById(R.id.delete_restaurant_page_rest_features_tw);
            makeReservation = (Button) findViewById(R.id.delete_make_reservation_button);
            showReservationList = (Button) findViewById(R.id.delete_show_reservation_list_button);
            approveButton = (Button) findViewById(R.id.delete_approve_button_for_admin);
            rejectButton=(Button) findViewById(R.id.delete_reject_button_for_admin);
            ...
            approveButton.setOnClickListener(new View.OnClickListener() {
                ...
                @Override
                public void onClick(View v) {
                    approveRestaurantDeleteRequest();
        ...
        public void approveRestaurantDeleteRequest(){
            Intent intent = getIntent();
            int RestaurantId = intent.getIntExtra("RestaurantId", 0);

            HashMap<String, String> body = new HashMap<>();
            body.put("restaurantID",String.valueOf(RestaurantId));
            Call<Restaurant> call = RetrofitClient.getUrls().approveDeleteRestaurantRequest(body);
            call.enqueue(new Callback<Restaurant>() {
                @Override
                public void onResponse(Call<Restaurant> call, Response<Restaurant> response) {
                    if(response.isSuccessful()){
                        Toast.makeText(getApplicationContext(), "Restaurant  başarılı bir şekilde silindi", Toast.LENGTH_SHORT)
                                .show();
        ...
        public void rejectRestaurantDeleteRequest(){
            Intent intent = getIntent();
            int RestaurantId = intent.getIntExtra("RestaurantId", 0);

            HashMap<String, String> body = new HashMap<>();
            body.put("restaurantID",String.valueOf(RestaurantId));
            Call<Restaurant> call = RetrofitClient.getUrls().rejectDeleteRestaurantRequest(body);
            call.enqueue(new Callback<Restaurant>() {
                @Override
                public void onResponse(Call<Restaurant> call, Response<Restaurant> response) {
                    if(response.isSuccessful()){
                        Toast.makeText(getApplicationContext(), "Restaurant  silme isteği geri çevirildi", Toast.LENGTH_SHORT)
                                .show();
                        new Handler().postDelayed(new Runnable() {
                            @Override
                            public void run() {
